...
function index(req, res) {
  ...
  req.getConnection((err, conn) => {      
      conn.query('SELECT 	d.id_dosis as id, d.hora as hora, d.cantidad as cantidad, m.nombre_Medicamento as medicamento, m.nombre_Comun as nombre_Comun FROM dosis d, medicamentos m WHERE	d.id_medicamento = m.id_medicamento AND		d.id_usuario = 1 ORDER BY d.hora;', (err, tasks) => {
      if(err) {
...
function logs(req, res) {
  ...
  req.getConnection((err, conn) => {      
      conn.query('SELECT l.ID_Log, substr(l.fecha, 1, 10) as fecha, l.resultado as resultado, d.hora, m.nombre_medicamento as medicamento FROM log l, dosis d, medicamentos m WHERE l.ID_dosis = d.ID_dosis AND d.id_Medicamento = m.ID_Medicamento order by fecha, hora;', (err, tasks) => {
      if(err) {
